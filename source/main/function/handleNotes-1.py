...
def handleNotes(param):
    if request.method == "GET":
        try:
            notes = db.session.execute(
                text(
                    "Select * from (select * from notes where notes.idUser={} and notes.inArchived=1) as b inner join datas on b.idNote=datas.idNote".format(
                        param
                    )
                )
            )
    ...
    if request.method == "POST":
        try:
            ...
            date_dueAt = None
            if json["dueAt"]:
                date_dueAt = datetime.strptime(json["dueAt"], "%d/%m/%Y %H:%M %p %z")
            date_rmAt = None
            if json["remindAt"]:
                date_rmAt = datetime.strptime(json["remindAt"], "%d/%m/%Y %H:%M %p %z")
            lockPass = None
        ...
        try:
            ...
            for key in list(json.keys()):
                ...
                if key == "data":
                    ...
                    if json["type"] == "text":
                        note_data = Datas.query.filter(Datas.idNote == param).first()
...
def getNotesShare(nid):
    if request.method == "GET":
        try:
            note = Notes.query.filter(Notes.linkNoteShare == nid).first()
            return {
                "status": 200,
                "message": "Note get successfully",
                "note": getNote(note.idNote),
            }
        except Exception as e:
            print(e)
            return make_response(
