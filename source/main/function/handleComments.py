...
def getCommentnotes(idNote):
    if request.method == "GET":
        try:
            ...
            data = []
            comments = (
                Comments.query.filter(Comments.idNote == idNote)
                .order_by(Comments.idNote.desc())
                .all()
            )
            total_reply = 0
            for comment in comments:
                if comment.parent_id == 0:
                    ...
                    like = Favorites.query.filter(
                        Favorites.idComment == comment.id, Favorites.type.like("like")
                    ).all()
                    if like:
                        like_count = len(like)
                    else:
                    ...
                    dislike = Favorites.query.filter(
                        Favorites.idComment == comment.id,
                        Favorites.type.like("dislike"),
                    ).all()
                    if dislike:
                    ...
                    favoriteType = None
                    if current_user_id:
                        isFavorite = Favorites.query.filter(
                            Favorites.idUser == current_user_id,
                            Favorites.idComment == comment.id,
                        ).first()
                        if isFavorite:
                    ...
                    comment_parse["favoriteType"] = (
                        favoriteType if favoriteType else None
                    )
                    # reply_comments = Comments.query.filter(
                    #     Comments.parent_id == comment.id
                    # ).order_by(Comments.id.desc())
                    reply_comments = Comments.query.filter(
                        Comments.parent_id == comment.id
                    ).order_by(Comments.sendAt.asc())
                    if reply_comments:
                        list_reply = []
                        for reply in reply_comments:
                            reply_user = Users.query.filter(
                                Users.id == reply.idUser
                            ).first()
                            # favorites like
                            reply_like = Favorites_Reply.query.filter(
                                Favorites_Reply.idReply == reply.id,
                                Favorites_Reply.type.like("like"),
                            ).all()
                            print("_____replyLike______", str(reply_like))
                            if reply_like:
                                likeCount = len(reply_like)
                            else:
                                likeCount = 0
                            # dislike
                            reply_dislike = Favorites_Reply.query.filter(
                                Favorites_Reply.idReply == reply.id,
                                Favorites_Reply.type.like("dislike"),
                            ).all()
                            if reply_dislike:
                                disLikeCount = len(reply_dislike)
                            else:
                                disLikeCount = 0

                            reply_data = {
                                "id_reply": reply.id,
                                "idUser": reply_user.id,
                                "idNote": reply.idNote,
                    ...
                    print(comment)
            return {"status": 200, "data": data, "total_replyComment": total_reply}
        except Exception as e:
            print(e)
            return make_response(jsonify({"status": 300, "message": str(e)}), 300)
